...
app.post('/api/generate', upload.single('excelFile'), async (req, res) => {
  try {
    ...
    if (paperType === 'mid1') {
      // Mid 1: 4 MCQs + 4 FIBs from Unit 1, 4 MCQs + 4 FIBs from Unit 2, 2 MCQs + 2 FIBs from Unit 3
      if (multipleChoiceByUnit[1].length < 4 || multipleChoiceByUnit[2].length < 4 || multipleChoiceByUnit[3].length < 2) {
        return res.status(400).json({
          error: `Insufficient multiple-choice questions for Mid 1: Need 4 from Unit 1 (found ${multipleChoiceByUnit[1].length}), 4 from Unit 2 (found ${multipleChoiceByUnit[2].length}), 2 from Unit 3 (found ${multipleChoiceByUnit[3].length})`
        });
      }
      if (fillInTheBlankByUnit[1].length < 4 || fillInTheBlankByUnit[2].length < 4 || fillInTheBlankByUnit[3].length < 2) {
        return res.status(400).json({
          error: `Insufficient fill-in-the-blank questions for Mid 1: Need 4 from Unit 1 (found ${fillInTheBlankByUnit[1].length}), 4 from Unit 2 (found ${fillInTheBlankByUnit[2].length}), 2 from Unit 3 (found ${fillInTheBlankByUnit[3].length})`
        });
      }
      ...
      selectedQuestions = [
        ...
        ...shuffleArray([...multipleChoiceByUnit[3]]).slice(0, 2), // 2 MCQs from Unit 3
      ...
      console.log('Mid 1 Selection Breakdown:', {
        'Q1-Q4 (MC, Unit 1)': selectedQuestions.slice(0, 4).map(q => ({ question: q.question, unit: q.unit, type: q.type, options: [q.optionA, q.optionB, q.optionC, q.optionD] })),
        'Q5-Q8 (MC, Unit 2)': selectedQuestions.slice(4, 8).map(q => ({ question: q.question, unit: q.unit, type: q.type, options: [q.optionA, q.optionB, q.optionC, q.optionD] })),
        'Q9-Q10 (MC, Unit 3)': selectedQuestions.slice(8, 10).map(q => ({ question: q.question, unit: q.unit, type: q.type, options: [q.optionA, q.optionB, q.optionC, q.optionD] })),
        'Q11-Q14 (FIB, Unit 1)': selectedQuestions.slice(10, 14).map(q => ({ question: q.question, unit: q.unit, type: q.type })),
        'Q15-Q18 (FIB, Unit 2)': selectedQuestions.slice(14, 18).map(q => ({ question: q.question, unit: q.unit, type: q.type })),
        'Q19-Q20 (FIB, Unit 3)': selectedQuestions.slice(18, 20).map(q => ({ question: q.question, unit: q.unit, type: q.type }))
      });
      ...
      console.log('Mid 2 Selection Breakdown:', {
        'Q1-Q2 (MC, Unit 3)': selectedQuestions.slice(0, 2).map(q => ({ question: q.question, unit: q.unit, type: q.type, options: [q.optionA, q.optionB, q.optionC, q.optionD] })),
        'Q3-Q6 (MC, Unit 4)': selectedQuestions.slice(2, 6).map(q => ({ question: q.question, unit: q.unit, type: q.type, options: [q.optionA, q.optionB, q.optionC, q.optionD] })),
        'Q7-Q10 (MC, Unit 5)': selectedQuestions.slice(6, 10).map(q => ({ question: q.question, unit: q.unit, type: q.type, options: [q.optionA, q.optionB, q.optionC, q.optionD] })),
